...
| `recharts` -> `lodash` | high | browser bundle transitive | Potentially actionable. Recharts is imported by the chart component; the audit flags Lodash code-injection/prototype-pollution behavior. A safe fix is not established without a Recharts compatibility test; do not force an override blindly. |
| `recharts` -> `react-smooth` -> `@babel/runtime` | moderate | browser bundle transitive | Runtime dependency, but the advisory concerns generated-code RegExp complexity. Review with the Recharts upgrade decision. |
