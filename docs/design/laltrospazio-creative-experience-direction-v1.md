...
**Color logic:** a restrained, mostly neutral palette (the book's own cream/off-white/charcoal base is a legitimate, non-proprietary starting point — it's simply good print design, not a brand asset) with one or two functional accent colors reserved *only* for status meaning (e.g., a distinct, consistent treatment for "current" vs. "historical" vs. "needs verification") — color as information, not mood.
...
**Image behaviour:** every image carries a visible kind-marker (a small, consistent label or border convention — CURRENT / ARCHIVAL / PROCESS / DIAGRAM / ARTWORK / EVENT) and, where dated, its date, rendered legibly, not buried in metadata only a screen reader sees. No single filter/grade applied uniformly across kinds.
...
**Color strategy:** a restrained neutral base (the book's cream/off-white/charcoal register is a legitimate starting point precisely because it is unbranded, print-agnostic good design — not because "the book uses it"); one or two functional accent colors reserved strictly for status meaning (current/verified vs. needs-confirmation vs. historical); modest, disciplined per-section warmth variation (Territory B's graft) so History doesn't feel like it belongs to a different, colder site than Visit.
...
**What they must not share:** the book's specific print-editorial graphic system — its compressed rotated typography, its exact cream/charcoal photo treatment as a fixed brand palette (rather than as one legitimate, unbranded starting point independently arrived at, per §14/§19), its dotted-line cover motif, its constellation mural, its blueprint linework, its specific page-turn pacing. None of these are brand standards; all of them are one publication's own creative choices, made for print, for a specific physical object, at a specific moment.
...
3. **DECISION:** Is the owner open to the recommended palette starting point (the book's own cream/off-white/charcoal register, arrived at independently in Territory A as unbranded good print design, per §14/§19) even though it will look familiar to anyone who has also seen the book?
   **Why it matters:** this document argues the resemblance is coincidental convergence on sound, neutral design rather than borrowed brand skin, but the owner may reasonably want more visual distance from the book than that argument allows, especially given the "related by logic, not by skin" standard this document itself sets.
   **Recommended default:** proceed with the neutral palette as a starting hypothesis, to be pressure-tested (and changed if it reads as imitation rather than convergence) once real page mockups exist in the visual-design phase.
   **Consequence of deferring:** no loss; palette is explicitly deferred to the next phase (§27) regardless.
...
| 9 | Does archive/current distinction remain clear? | **PASS, pending prototype validation** | §19, §21; flagged in §28 item 1 as the single most important thing to actually test before trusting it |
...
**15/15.** Two gates (4 and 9) are marked PASS with an explicit named risk or dependency rather than an unqualified pass, because both are the specific places this document's own analysis (§17, §28) identifies as needing continued discipline (gate 4) or empirical prototyping (gate 9) rather than being self-evidently solved by argument alone.
