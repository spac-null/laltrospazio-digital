...
**Availability status (renamed from "current operational condition" after the real-content stress test, see the content-model-real-content-test-v1 document §18):** not a stored field at all — a derived read, computed from lifecycle (is this place ACTIVE?), operating pattern plus any bound SEASON's current window (if SEASONAL), and any ACTIVE-status NOTICE scoped to this place. This answers only "is this place, in general terms, currently part of the organisation's active operation" (e.g. "Via Nazario — normally operating, currently affected by a stated closure" / "Parco — seasonally active for summer 2026") — it is explicitly **not** the same question as "is it open at this exact clock moment," which requires a fourth input this axis does not include.
**Schedule (a separate, optional fact):** a place's normal opening hours, when known. Presently `NEEDS-VERIFICATION` for both Via Nazario and Parco (Current Public State v1 §2, §5) — its absence must never be treated as license to assume availability status alone answers "open right now."
**Real-time open/closed state is out of scope for this content model.** Deriving "is the door open at this literal moment" requires a populated Schedule plus an actual clock-time check — that computation belongs to a later implementation/display layer, never to this semantic model. Availability status licenses a claim like "Parco is active this summer"; it never licenses "Parco is open right now," and no field in this model computes the latter.
**Public/internal:** Public.
**Relationships:** operated by ORGANISATION (see §13's clarification of what "operator" does and does not assert for a public-space place like Parco); described by one ACCESS PROFILE; hosts EVENTs; may be bound to a SEASON (if operating_pattern = SEASONAL); may be the subject of a HISTORY ENTRY once HISTORICAL.
...
**Relationships:** scope_ref is polymorphic — PROJECT, WORK, PLACE, or ORGANISATION-wide — and, **unlike NOTICE's deliberately single-target rule, may name more than one scope at once** (clarified after the real-content stress test surfaced the Comune di Bologna's Parco pact, which is genuinely scoped to both a WORK and a PLACE simultaneously, §6). This is safe precisely because PARTNERSHIP's scope does not carry NOTICE's inference risk — naming two explicit scopes is not the same failure mode as inferring a wider one from a single narrow fact, which is what NOTICE's single-target rule exists to prevent.
...
**Axis 3 — availability status, which is never stored.** Corrected by the real-content stress test (`laltrospazio-content-model-real-content-test-v1.md` §18): what this axis actually is is not one concept but a derived read combining three genuinely distinct, more granular questions, plus two further questions this document deliberately keeps outside the derived read altogether:

1. **Existence/lifecycle** — does this place currently exist as part of the practice at all? (`lifecycle = ACTIVE`?)
2. **Seasonal availability** — if `operating_pattern = SEASONAL`, is there a `CURRENT`-status `SEASON` window bound to this place right now?
3. **Exceptional status** — is there an `ACTIVE`-status `NOTICE` scoped to this place overriding the default reading?
4. **Schedule** — what are this place's normal opening hours? A separate, optional, independently-verified fact (§3.1), *not* one of the three inputs to availability status, and presently unresolved for both places tested.
5. **Real-time open/closed state** — is it open at this exact clock moment? This requires (4) plus an actual time check, and this content model does not compute it at all; it is out of scope, reserved for a later display/implementation layer.

Availability status is derived from (1)–(3) only. It answers "is this place, in general terms, part of the organisation's currently active operation" — never "is it open right this second," which would additionally require (4) and (5). Conflating the two was a real, if unstated, risk in the model's original phrasing here (a single term, "current operational condition," could too easily be read as answering both); the corrected framing keeps them structurally apart so no future implementer derives a real-time open/closed claim from data that cannot support one.

**Worked against the two current examples:** Via Nazario — lifecycle `ACTIVE`, operating_pattern `YEAR-ROUND`, currently reading as closed purely because an `ACTIVE` `NOTICE` (12–26 August 2026, with its own `effective_start`/`effective_end` and automatic expiry) says so; the moment that notice expires, the derived read reverts with nothing to manually flip back. Its schedule (4) is unresolved, so no real-time claim (5) can ever be made about it regardless. Parco — lifecycle `ACTIVE`, operating_pattern `SEASONAL`, currently reading as seasonally active because "now" falls inside a `CURRENT`-status `SEASON` window bound to it — this licenses "Parco is active this summer," not "Parco is open right now"; Parco's schedule (4) is equally unresolved. Neither place's lifecycle value has changed at all in either case — a lifecycle field that had to flip every time a venue closed for a week or a season ended would invite exactly the staleness risk Task 12 warns about (a status a human has to remember to revert); deriving the current read instead means there is nothing to forget.

**What "operator" does and does not assert.** Via Nazario's operator relationship (Cooperativa L'Altro Spazio) is straightforward: the cooperative leases and runs the venue directly. Parco is a different case, and the real-content stress test made this concrete enough to state explicitly: Parco is a public park, not a leased venue — the cooperative does not own or manage the site itself, and PLACE.operator here means specifically "the ORGANISATION currently responsible for L'Altro Spazio's activity at this place," not facility ownership or exclusive site control. Site-level authority at Parco is a separate fact, carried by the PARTNERSHIP with the Comune di Bologna (§15), not by the operator field. This distinction matters precisely because collapsing the two would overstate what the cooperative controls at a public park it merely holds a collaboration pact for.
...
**`NOT-PRESENT` requires positive confirming evidence of absence, never mere silence or assumption.** The real-content stress test found a real trap here worth naming explicitly: "no evidence of X anywhere in the repository" is not the same fact as "X is confirmed absent," and only the latter is `NOT-PRESENT`. Via Nazario's induction-loop question — genuinely never checked, in either direction — is `condition = UNKNOWN` with `verification_status = NEEDS-VERIFICATION`, not `NOT-PRESENT`; even a source that labels something "not available (assumed)" describes an unconfirmed assumption, not a confirmed absence, and must be modelled the same way. `NOT-PRESENT` is reserved for cases where someone actually checked and found the feature genuinely does not exist.
...
- **VERIFIED-CURRENT** — established by an owner or otherwise authoritative current source
...
**This publication-status axis is deliberately independent of the freshness axis (§18), and the real-content stress test found the two had drifted into overlapping language here.** VERIFIED-CURRENT describes the *quality of the source* that established a fact, not *how recently* it was reconfirmed — a fact can be VERIFIED-CURRENT (properly established by a good source) and simultaneously STALE (that establishment is old enough to need reconfirming) at the same time; the two axes combine, and a display layer must honour both (a STALE VERIFIED-CURRENT fact still must not render as though freshly confirmed, per the corrected Invariant 4 in §21). Each field additionally carries `source`, `authority`, `verified_date`, and `recheck_after` — the latter two feed §18's freshness computation, not this status value.
...
- **REVIEW_DUE** — now is past review_after, within a further staleness threshold
- **STALE** — now is past that further threshold as well

**Neither threshold is a single global constant.** The real-content stress test (`laltrospazio-content-model-real-content-test-v1.md` §19) found the original phrasing here implied one universal "grace period" across the whole model, which is not justified by anything in the evidence base — a WORK's reasonable recheck cadence (e.g. a decade-old format like Cena al Buio) has no reason to match a CONTACT CHANNEL's, and neither has a reason to match an ACCESS PROFILE dimension's. `review_after` and the further point at which REVIEW_DUE becomes STALE are each set **per content family, and may be set per field within a family**, not asserted here as one duration for the whole model — this document defines the *mechanism* (deterministic date arithmetic against stored thresholds) and deliberately leaves the *specific durations* to a later editorial-policy decision, made per family once real content volume and update cadence are better understood. What remains fixed regardless of duration: both thresholds are plain date arithmetic on stored fields — there is no interpretation, judgment, or content-aware reasoning involved in computing which of the three conditions applies at any moment, only in choosing where each family sets its own thresholds.
...
- **state:** lifecycle (PROPOSED/ACTIVE/HISTORICAL) — the existence axis; operating_pattern (YEAR-ROUND/SEASONAL/EVENT-BASED) — a separate, independent axis; schedule (regular opening hours, optional, independently verified — presently unresolved for all current places tested) — a separate, optional fact, never conflated with the other two; availability status (derived, never stored, from lifecycle + operating_pattern + bound SEASON + ACTIVE NOTICE — §5); real-time open/closed state is not modelled at all — out of scope for this document
...
4. **Generalized by the real-content stress test** (`laltrospazio-content-model-real-content-test-v1.md` §7, §19 — the original wording here covered only ACCESS PROFILE dimensions, which left a real gap for every other entity carrying independent freshness metadata): any entity or field whose freshness condition (§18) is REVIEW_DUE or STALE must never render as though it were VERIFIED-CURRENT/freshly confirmed, regardless of what its own real-world `status` says — this applies to ACCESS PROFILE dimensions, WORK, SERVICE, PARTNERSHIP, and CONTACT CHANNEL alike. Concretely: a SERVICE whose `status` is honestly `CURRENT-SERVICE` (because that was the last-confirmed real-world state) but whose freshness condition has gone STALE must not render an active booking/contact call-to-action with the same confidence as a freshly-verified one. The exact fallback presentation is a display-policy decision for a later phase, but silent continuation of a stale claim — on any entity — is never permitted.
...
16. A PLACE's availability status must never be a separately stored, independently editable field — it must always be computed at render time from lifecycle + operating_pattern + any bound SEASON + any ACTIVE-scope NOTICE, so it can never drift out of sync with those sources.
17. A PLACE's availability status must never be rendered or interpreted as a real-time open/closed claim ("open right now") unless a Schedule fact is populated and a separate, explicit real-time check is performed by the display/implementation layer — this content model computes availability status only, never real-time open state, and "seasonally/currently active" must never be presented as equivalent to "open at this moment."
...
1. **RESOLVED by the real-content stress test** (`laltrospazio-content-model-real-content-test-v1.md` §17): **keep SEASON as its own lightweight entity.** This was originally left open pending real content; instantiating both the 2026 Parco summer season and comparing it against the working paper's own documented 2017-era Parco/Cineporto season (still Farm-branded, pre-cooperative) showed SEASON carrying real, non-redundant information neither PLACE nor WORK can hold on its own — its own status ladder, its own date window (even when incomplete), and a place for year-specific partnership/branding facts to attach without disturbing the durable PLACE or WORK records underneath. No longer an open decision.
