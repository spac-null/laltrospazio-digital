...
import { META_REPORT_FILE, readStoredMetaToken } from "./meta-env.mjs";
import { runMetaProbe } from "./meta-probe-lib.mjs";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

try {
  const { accessToken } = readStoredMetaToken(root);
  const report = await runMetaProbe({ accessToken });
